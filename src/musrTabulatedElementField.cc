...
void musrTabulatedElementField::addFieldValue3D(const G4double point[4],
				      G4double *field ) const
{
//  G4cout<<"musrTabulatedElementField::addFieldValue3D()"<<G4endl;
  ...
  if ( x>=minimumx && x<maximumx-0.5 &&
       y>=minimumy && y<maximumy-0.5 && 
       z>=minimumz && z<maximumz-0.5 ) {
    ...
    if ((xindex<0)||(xindex>(nx-2))) {
      std::cout<<"SERIOUS 3D PROBLEM:  xindex out of range!  xindex="<<xindex<<"   x="<<x<<"  xfraction="<<xfraction<<"   maximumx="<<maximumx<<std::endl;
      if (xindex<0) xindex=0;
      else xindex=nx-2;
    }
    if ((yindex<0)||(yindex>(ny-2))) {
      std::cout<<"SERIOUS 3D PROBLEM:  yindex out of range!  yindex="<<yindex<<"   y="<<y<<"  yfraction="<<yfraction<<"   maximumy="<<maximumy<<std::endl;
      if (yindex<0) yindex=0;
      else yindex=ny-2;
    }
    if ((zindex<0)||(zindex>(nz-2))) {
      std::cout<<"SERIOUS 3D PROBLEM:  zindex out of range!  zindex="<<zindex<<"   z="<<z<<"  zfraction="<<zfraction<<"   maximumz="<<maximumz<<std::endl;
...
//  G4cout<<"musrTabulatedElementField::addFieldValue2D()"<<G4endl;
...
//      (strcmp(fieldTableType,"2D_OperaXY"))||(strcmp(fieldTableType,"2DEf")==0)) {
  ...
  if ( x>=minimumx && x<maximumx &&
    ...
    if ((xindex<0)||(xindex>(nx-2))) {
  G4cout<<"musrTabulatedElementField::addFieldValue2D()"<<G4endl;
     G4cout<<"x: "<<x<<", z: "<<z<<G4endl;
     G4cout<<"minimumx: "<<minimumx<<", minimumz: "<<minimumz<<", maximumx: "<<maximumx<<", maximumz: "<<maximumz<<G4endl;
     G4cout<<"xfraction: "<<xfraction<<", zfraction: "<<zfraction<<G4endl;
     G4cout<<"xlocal: "<<xlocal<<", zlocal: "<<zlocal<<G4endl;
     G4cout<<"xindex: "<<xindex<<", zindex: "<<zindex<<G4endl;
      std::cout<<"SERIOUS 2D PROBLEM:  xindex out of range!  xindex="<<xindex<<"   x="<<x<<"  xfraction="<<xfraction<<std::endl;
      if (xindex<0) xindex=0;
      else xindex=nx-2;
    }
    if ((zindex<0)||(zindex>(nz-2))) {
  G4cout<<"musrTabulatedElementField::addFieldValue2D()"<<G4endl;
     ...
     G4cout<<"xlocal: "<<xlocal<<", zlocal: "<<zlocal<<G4endl;
     G4cout<<"xindex: "<<xindex<<", zindex: "<<zindex<<G4endl;
      std::cout<<"SERIOUS 2D PROBLEM:  zindex out of range!  zindex="<<zindex<<"   z="<<z<<"  zfraction="<<zfraction<<std::endl;
      if (zindex<0) zindex=0;
      else zindex=nz-2;
    }
